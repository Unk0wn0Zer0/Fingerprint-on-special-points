    ...
    public partial class VisualFingerprintMatchingFrm : Form
    {
        ...
        private void ShowResults(double matchingScore, List<MinutiaPair> matchingMtiae)
        {
            if (matchingScore == 0 || matchingMtiae == null)
                MessageBox.Show(string.Format("Совпадение на: {0}.", matchingScore));
            else
            {
                ...
                MessageBox.Show(string.Format("Совпадение на: {0}. Соответсвтующие части: {1}.", matchingScore,
                                              matchingMtiae.Count));
            }
        }
        ...
        private void btnMatch_Click(object sender, EventArgs e)
        {
            if (qImage == null)
            {
                MessageBox.Show("Не загружен файл с отпечатком пальца!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            if (tImage == null)
            {
            ...
            if (minutiaMatcher != null)
            {
                ...
                if (qFeatures is List<Minutia> && tFeatures is List<Minutia>)
                {
                    ...
                    if (score == 0 || matchingMtiae == null)
                        MessageBox.Show(string.Format("Совпадение на: {0}.", score));
                    else
                    {
                        List<Minutia> qMtiae = new List<Minutia>();
                        List<Minutia> tMtiae = new List<Minutia>();
                        ...
                        MessageBox.Show(string.Format("Совпадение на: {0}. Соответсвтующие части: {1}.", score,
                                                      matchingMtiae.Count));
                    }
                }
                else
