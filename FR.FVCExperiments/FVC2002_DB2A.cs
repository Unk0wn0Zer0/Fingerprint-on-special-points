...
{
    /// <summary> 
    ///     Allows testing a fingerprint matching algorithm in database DB2_A of competition FVC2004.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    ///     </para>
    /// </remarks>
    public class FVC2002_DB2A : IMatchingExperiment
    {
        #region public
        ...
        private List<object> GetFeatures(out List<string> imgCaptions)
        {
            ...
            for (int i = 1; i <= 100; i++)
                for (int j = 1; j <= 8; j++)
                {
                    string label = string.Format("db2_{0}_{1}", i, j);
